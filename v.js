...
// ExpressVPN
const EXPRESSVPN_CLI = process.env.EXPRESSVPN_CLI || 'C:\\Program Files (x86)\\ExpressVPN\\services\\ExpressVPN.CLI.exe';
const EXPRESS_JP_SERVERS = ['Japan - Osaka', 'Japan - Shibuya', 'Japan - Tokyo', 'Japan - Yokohama'];
...
const CART_XPATH = '//a[@href="https://t3.basket.step.rakuten.co.jp/rms/mall/bs/cartall/?l-id=pc_header_func_cart"]';
...
function readAccounts() {
  if (!fs.existsSync(ACCOUNTS_FILE)) { console.error('Không tìm thấy file accounts.txt:', ACCOUNTS_FILE); process.exit(1); }
  return fs.readFileSync(ACCOUNTS_FILE, 'utf8').split(/\r?\n/).map(l => l.trim()).filter(Boolean)
    .map(line => {
      const [email, password, changePassword] = line.split('||').map(s => s?.trim());
...
function readHotmails() {
  ...
  return fs.readFileSync(HOTMAIL_FILE, 'utf8').split(/\r?\n/)
    .map(l => l.trim())
...
function parseProxyLine(line) {
  let s = line.trim(); let scheme = '';
  if (/^https?:\/\//i.test(s)) { const m = s.match(/^(https?:\/\/)/i); scheme = m ? m[1].toLowerCase() : ''; s = s.replace(/^[a-z]+:\/\//i, ''); }
  const parts = s.split('@'); let auth = null, hostPart = s;
  if (parts.length === 2) { auth = parts[0]; hostPart = parts[1]; } else if (parts.length > 2) return null;
  const m = hostPart.match(/^(.+?):(\d{1,5})$/); if (!m) return null;
  const host = m[1], port = m[2]; const server = (scheme || 'http://') + host + ':' + port;
  let username, password;
  if (auth) { const am = auth.match(/^(.+?):(.*)$/); if (am) { username = decodeURIComponent(am[1]); password = decodeURIComponent(am[2]); } }
  const obj = { server }; if (username) obj.username = username; if (password) obj.password = password; return obj;
}
...
function appendHeaderFiles() {
  ...
  fs.writeFileSync(path.resolve(__dirname, 'no_point_account.txt'), hdr, 'utf8');
  fs.writeFileSync(path.resolve(__dirname, 'accdie.txt'), hdr, 'utf8');
}

function execSafe(cmd) {
  try { return execSync(cmd, { stdio: 'pipe' }).toString(); }
  catch (e) { const stderr = e && e.stderr ? e.stderr.toString() : e.message; throw new Error(stderr || 'exec failed'); }
}
async function connectExpress(server) {
  try { execSafe(`"${EXPRESSVPN_CLI}" disconnect`); } catch (e) { }
  console.log('Đang kết nối ExpressVPN ->', server);
  execSafe(`"${EXPRESSVPN_CLI}" connect "${server}"`);
  console.log('Đã kết nối ExpressVPN tới', server, `— đợi ${EXPRESS_CONNECT_WAIT_MS || EXPRESS_CONNECT_WAIT_MS}ms`);
  await sleep(EXPRESS_CONNECT_WAIT_MS);
}

// stealth helpers
function randomUserAgent() {
  const major = randInt(100, 120), build = randInt(4200, 5400), patch = randInt(10, 200);
...
async function detectScreenSize() {
  const tmp = await chromium.launch({ headless: false, args: ['--window-size=800,600'] });
  const ctx = await tmp.newContext();
  const page = await ctx.newPage();
  await page.goto('about:blank');
  const dims = await page.evaluate(() => ({ w: window.screen.availWidth || window.screen.width, h: window.screen.availHeight || window.screen.height }));
  await tmp.close();
  return dims;
}
...
function computeGridForConcurrency(n, screenW, screenH) {
  ...
  while ((winW * cols + spacingW * (cols - 1) > screenW || winH * rows + spacingH * (rows - 1) > screenH) && cols < n) {
    cols++;
...
async function waitForLoginResult(page, timeoutMs) {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      // English fail
      const failEn = await page.$('//div[contains(text(),"Username and/or password are incorrect")]');
      if (failEn) {
        await page.waitForTimeout(randInt(FAIL_CLOSE_MIN_MS, FAIL_CLOSE_MAX_MS));
        return 'fail_en';
      }

      // Japanese fail (new selectors)
      const failJa1 = await page.$('//div[contains(text(),"ユーザIDまたはパスワードが正しくありません")]');
      const failJa2 = await page.$('//div[contains(@class,"error-message") and contains(text(),"ユーザIDまたはパスワード")]');
      const failJa3 = await page.$('//div[@data-testid="error-message" and contains(text(),"ユーザIDまたはパスワード")]');
...
async function readCodeFromEmail(email, password, type = 'password_reset') {
  console.log("readCodeFromEmail", email, password, type);
              ...
              msg.on("body", (stream) => {
                simpleParser(stream, (err, parsed) => {
                  ...
                  if (!err && parsed.subject?.includes("Rakuten")) {
                    ...
                    if (type === 'password_reset' && parsed.subject?.includes("Password Reset")) {
                      match = text.match(/following code:\s*([a-zA-Z0-9]{20,})/i);
                      if (!match) match = text.match(/token=([a-zA-Z0-9]{20,})/i);
                    } else if (type === 'otp') {
                      match = text.match(/verification code is as follows:\s*(\d{6})/i);
...
async function changePasswordFlow(page, email, password, newPassword, tag) {
  try {
    ...
    const checkboxInfo = await page.evaluate(async () => {
      ...
      const challenge = challenger.challengerMain.cores.values().next()?.value?.challenge;
      if (!challenge) return null;
      const metadata = JSON.parse(challenge.mdata);
      let realId = null;
      for (const [id, info] of Object.entries(metadata)) {
        if (info.input === "checkbox" && info.render === 0) { realId = id; break; }
      }
      const el = Array.from(challenge.inputFieldsDiv.childNodes).find(item => item.id === realId);
      if (!el) return null;
    ...
    if (checkboxInfo && checkboxInfo.width > 0) {
      await page.mouse.move(checkboxInfo.x + checkboxInfo.width / 2, checkboxInfo.y + checkboxInfo.height / 2, { steps: 15 });
    ...
    const sendEmailButton = page.getByText("Send email", { exact: false }).first();
    await sendEmailButton.click();
    await sleep(5000);

    const enterCode = page.getByRole("button", { name: "Enter the email verification code" });
    ...
    await sleep(1000);
    await page.locator('input[aria-label="Re-enter password"]').fill(newPassword);
    await sleep(1000);
    ...
    try {
      const successMsg = await page.getByText("Password successfully reset", { exact: false }).isVisible();
      if (successMsg) {
...
async function changeEmail(page, email, password, hotmailString, tag) {
  try {
    ...
    await page.goto('https://profile.id.rakuten.co.jp/account-security', { timeout: 60000 });
    ...
    try {
      await page.waitForSelector('[data-qa-id="email-edit-field"]', { timeout: 15000 });
    ...
    try {
      ...
      await sleep(2000);
      await page.waitForSelector('input[aria-label="Verification code"]', { timeout: 15000 });
      ...
      return { success: false, message: `OTP verification error: ${e.message}` };
    }
    ...
    // Click submit button
    try {
      await page.waitForSelector('[data-qa-id="submit-update-email"]', { timeout: 10000 });
    ...
    if (refreshToken && clientId) {
      const hotmailConfig = { email: newEmail, password: newEmailPass, refreshToken, clientId };
...
async function checkPoints(page, tag) {
  ...
  try {
    ...
    const pointsUrl = 'https://point.rakuten.co.jp/?l-id=top_normal_myrakuten_point';    // Retry navigation
    let navSuccess = false;
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        await page.goto(pointsUrl, { waitUntil: 'networkidle', timeout: 30000 });
        navSuccess = true;
        break;
      } catch (e) {
        if (attempt === 1) {
    ...
    try {
      await page.waitForSelector('.point-gadget-display-point .point_num', { timeout: 5000 });
    ...
    try {
      await page.waitForSelector('#js-pointBankTotalBalance .point_num', { timeout: 5000 });
    ...
    try {
      await page.goto('https://my.rakuten.co.jp/?l-id=pc_footer_account', { waitUntil: 'networkidle', timeout: 30000 });
      await sleep(3000);

      await page.waitForSelector('[data-ratid="available-rcash-area"]', { timeout: 5000 });
      const el = await page.$('[data-ratid="available-rcash-area"] span:nth-child(2)');
      if (el) {
        const text = await el.textContent();
        const cleaned = text.trim().replace(/[^\d]/g, '');
        if (cleaned) {
...
function makeContextOptions(proxyObj) {
  return {
    ...
    ...(proxyObj ? { proxy: { server: proxyObj.server, username: proxyObj.username, password: proxyObj.password } } : {})
  };
}
...
async function retryAction(actionFn, retries = 3, delayMs = 5000) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (msg.includes('Timeout') || msg.includes('net::ERR') || msg.includes('Navigation failed')) {
...
async function processAccountAtSlot(slotPos, slotSize, acc, idx, useProxy, proxies, useExpress, headlessReq, hotmail = null) {
  ...
  const launchProxy = (useProxy && proxies.length > 0) ? proxies[(idx - 1) % proxies.length] : null;
  if (launchProxy) {
    launchOptions.proxy = { server: launchProxy.server };
    if (launchProxy.username) launchOptions.proxy.username = launchProxy.username;
    if (launchProxy.password) launchOptions.proxy.password = launchProxy.password;
  }
  ...
  try {
    ...
    fs.appendFileSync(path.resolve(__dirname, 'no_point_account.txt'), errorLine, 'utf8');
  ...
  try {
    ...
    if (acc.changePassword) {
      const cpRes = await changePasswordFlow(page, acc.email, acc.password, acc.changePassword, tag);
      if (cpRes.ok) {
        loginSuccess = true;
      } else {
        console.log(`❌ ${tag} → Đổi password thất bại: ${cpRes.message}`);
        // Fallback to regular login if changePassword failed? Or stop?
        // User said "thêm vào đầu", so let's try regular login if it fails.
      }
    }

    if (!loginSuccess) {
      // Regular login
      await retryAction(() => page.goto(LOGIN_URL, { waitUntil: 'domcontentloaded', timeout: TIMEOUT_MS }));
      await retryAction(() => page.waitForSelector('#user_id', { timeout: TIMEOUT_MS }));
      await retryAction(() => page.fill('#user_id', acc.email));
      await page.keyboard.press('Enter');
      await retryAction(() => page.waitForSelector('#password_current', { timeout: TIMEOUT_MS }));
      ...
      if (outcome === 'success') {
        ...
        fs.appendFileSync(path.resolve(__dirname, 'accdie.txt'), failLine, 'utf8');
        removeAccountFromFile(acc.email);
    ...
    if (loginSuccess) {
      console.log(`✅ ${tag} → Đã đăng nhập — đang kiểm tra điểm (Check-point)`);
      const pointsData = await checkPoints(page, tag);

      let changedEmailResult = null;
      if (hotmail) {
        console.log(`🔄 ${tag} → Đang thử đổi email...`);
        const changeResult = await changeEmail(page, acc.email, acc.password, hotmail, tag);
        if (changeResult.success) {
          console.log(`✅ ${tag} → Đã đổi email thành: ${changeResult.message}`);
          changedEmailResult = hotmail;
          removeHotmailFromFile(hotmail);
        } else {
          console.log(`❌ ${tag} → Đổi email thất bại: ${changeResult.message}`);
        }
      }
      ...
      if (pointsData.success) {
        ...
        fs.appendFileSync(path.resolve(__dirname, 'point_account.txt'), pointLine, 'utf8');
      } else {
        ...
        fs.appendFileSync(path.resolve(__dirname, 'no_point_account.txt'), noPointLine, 'utf8');
      }
...
(async () => {
  ...
  const modeAns = await ask('Chọn chế độ mạng (1 = Proxy, 2 = ExpressVPN) [default 1]: ');
  const showAns = await ask('Hiển thị trình duyệt? (1 = Hiển thị, 2 = Ẩn) [default 1]: ');
  const concAns = await ask(`Số worker song song? [default ${DEFAULT_CONCURRENCY}]: `);
  rl.close();

  const useProxy = (modeAns.trim() === '' || modeAns.trim() === '1');
  const useExpress = (modeAns.trim() === '2');
  const headlessReq = (showAns.trim() === '2');
  const concurrency = concAns.trim() ? Math.max(1, parseInt(concAns.trim(), 10)) : DEFAULT_CONCURRENCY;

  if (headlessReq) {
    console.log('LƯU Ý: Bạn đã chọn chế độ ẩn trình duyệt; vị trí xếp ô sẽ bị bỏ qua (mỗi tài khoản sẽ chạy riêng trình duyệt ẩn).');
  }
  ...
  if (accounts.length === 0) { console.log('Không tìm thấy tài khoản. Thoát chương trình.'); process.exit(0); }
  ...
  if (useExpress) {
    try { await connectExpress(pickRandom(EXPRESS_JP_SERVERS)); } catch (e) { console.warn('Kết nối Express lần đầu thất bại:', e.message || e); }
  }
  ...
  console.log(`Màn hình ${dims.w}x${dims.h} → lưới ${grid.cols}x${grid.rows}, mỗi ô ${grid.winW}x${grid.winH}`);
  ...
  for (let w = 0; w < actualConcurrency; w++) {
    ...
    const p = (async () => {
      while (true) {
        ...
        const hotmail = hotmails.length > 0 ? hotmails[(idx - 1) % hotmails.length] : null;
        // process account at this worker's slot (launch browser per-account)
        await processAccountAtSlot(slot, { width: slot.width, height: slot.height }, acc, idx, useProxy, proxies, useExpress, headlessReq, hotmail);
  ...
  console.log('Hoàn tất. Các file kết quả: point_account.txt, no_point_account.txt, accdie.txt');
  process.exit(0);
})();
